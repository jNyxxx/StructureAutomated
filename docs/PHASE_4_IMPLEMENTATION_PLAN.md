...
| P4-FinalManualDemoSmoke | Final local/manual demo smoke evidence | local Docker / evidence | **Blocked for final sign-off.** Core local/mock demo chain passed in Docker: login/session, dashboard, prospects/contact import, campaign create/detail/contact selection, grounded draft generation with 2 evidence rows, review queue, human approval, send-gate `passed`, mock send intent, outbound `mock_sent`, audit chain, billing/access, compliance/suppressions, deliverability, and outcomes surfaces. Blocker found only in the required logout -> login cycle: `POST /auth/logout` returns `revoked=1`, then `POST /auth/session` returns `401 AUTH_SESSION_REVOKED` because local mock tokens share one fixed provider session reference. No source fix was applied; open **P4-LocalMockAuthSessionCycle-Fix** or remove the logout/re-login step from the boss-demo script. See [evidence/phase-4-final-manual-demo-smoke.md](evidence/phase-4-final-manual-demo-smoke.md). |
