...
| Rate limits + abuse protection | Security | **P3-4a/P3-4b/P3-4c complete:** shared app-scoped limiter wired; auth/import/campaign/send/followup endpoint limits active; `sending.py`/`followups.py` per-request backend bug fixed; `RedisRateLimitBackend` added for production multi-worker correctness behind `RATE_LIMIT_BACKEND=redis`; production boot guard fails closed when Redis backend/URL is missing. Local/test default remains in-memory. No production deployment. See [evidence/phase-3-4a-rate-limits-abuse-protection-plan.md](evidence/phase-3-4a-rate-limits-abuse-protection-plan.md) and [evidence/phase-3-4c-redis-rate-limit-backend.md](evidence/phase-3-4c-redis-rate-limit-backend.md). |
