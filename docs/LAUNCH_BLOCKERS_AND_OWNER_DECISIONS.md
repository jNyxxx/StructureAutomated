...
| Rate limits + abuse protection | Security | **P3-4a inspection done:** infrastructure built (middleware, service, Protocol backend, policies, JobThrottle, 9 tests). **Critical bug:** `sending.py`/`followups.py` create new `InMemoryRateLimitBackend()` per request — `tenant_send`/`tenant_followup` limits are no-ops. Global middleware disabled. `POST /auth/session`, imports, campaigns unprotected. **P3-4b next:** shared `app.state` backend, fix no-op bug, per-endpoint `enforce()`, tests. Redis → P3-4c. See [evidence/phase-3-4a-rate-limits-abuse-protection-plan.md](evidence/phase-3-4a-rate-limits-abuse-protection-plan.md). |
