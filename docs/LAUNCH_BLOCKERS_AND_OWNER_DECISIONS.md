...
| Rate limits + abuse protection | Security | **P3-4a/P3-4b/P3-4c/P3-4d/P3-4e complete:** shared app-scoped limiter wired; auth/import/campaign/send/followup endpoint limits active; `sending.py`/`followups.py` per-request backend bug fixed; `RedisRateLimitBackend` added for production multi-worker correctness behind `RATE_LIMIT_BACKEND=redis`; production boot guard fails closed when Redis backend/URL is missing; local Redis runtime smoke proved real Redis counters, HTTP 429, tenant isolation, key PII safety, TTL reset; Redis-down now fails closed with sanitized `503 RATE_LIMIT_BACKEND_UNAVAILABLE`; `/ready` reports Redis state when Redis backend is configured. No production deployment. See [evidence/phase-3-4a-rate-limits-abuse-protection-plan.md](evidence/phase-3-4a-rate-limits-abuse-protection-plan.md), [evidence/phase-3-4c-redis-rate-limit-backend.md](evidence/phase-3-4c-redis-rate-limit-backend.md), [evidence/phase-3-4d-redis-runtime-smoke.md](evidence/phase-3-4d-redis-runtime-smoke.md), and [evidence/phase-3-4e-redis-readiness-error-hardening.md](evidence/phase-3-4e-redis-readiness-error-hardening.md). |
