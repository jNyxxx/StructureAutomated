...
| **P3-6** | Stripe / payment | real provider integration — **DEFERRED** | **P3-6a owner decision packet created; P3-6b config/readiness contract created; P3-6c safe defaults recorded; P3-6d webhook verification foundation added; P3-6e checkout/portal skeleton added. STOP GATE remains for real session creation, billing-state mutation, and money movement.** Defaults select Stripe direction, test mode first, mock billing default, manual-first-pilot option, self-serve checkout disabled by default, placeholder internal plans, 14-day trial, access defaults, 7-day payment failure grace, webhook event defaults, and owner/operator fallback roles. **P3-6f-prep (2026-06-30):** Stripe test-mode smoke preparation complete. Required concrete values (test secret refs, smoke approver, emergency-stop operator, Stripe CLI), test-mode gates, 13-step webhook smoke scenario, 10 hard stop conditions, evidence contract for P3-6h, and remaining slices P3-6g through P3-6k defined. Still open: secret resolution wiring (P3-6g), internal smoke (P3-6h), billing mutation design (P3-6i), mutation implementation (P3-6j), checkout/portal activation (P3-6k), final public prices, usage limits, named owners, and live-mode approval. No Stripe SDK/API call, real credentials, billing-state mutation, or money movement. See [evidence/phase-3-6f-prep-stripe-test-mode-smoke.md](evidence/phase-3-6f-prep-stripe-test-mode-smoke.md). |
| **P3-7** | Deployment runbook + staging | production readiness | **P3-7a inspection complete; P3-7b Docker hardening/build smoke complete; P3-7c staging env/secret template complete; P3-7d CI/CD release plan complete; P3-7d-impl validation gates complete; P3-7e-plan staging release runbook complete (2026-06-30); P3-7e-dryrun local staging rehearsal complete (2026-06-30):** current dev Docker/Compose preserved; production backend/frontend images build; staging config map and secret-ref template exist; CI validates app gates, safety guards, and production Docker builds without release behavior; staging runbook defines 20 prerequisites (all open), image/build procedure, migration one-off task, service startup, 21-item smoke checklist, 18 hard stop conditions, rollback plan, and 26-item evidence bundle. Staging/prod remain blocked on AWS account/region, deployment target, domains/TLS, Secrets Manager/KMS, RDS, Redis, backups/RPO/RTO, alerts, release automation approval, migration/rollback owners, cutover approver, registry target, and Clerk staging values. Future P3-7e actual deploy and P3-7f→P3-7g cover approved staging release, smoke evidence, and final production plan. **All boot-guard conditions must pass before any production boot.** See [evidence/phase-3-7e-staging-release-runbook-plan.md](evidence/phase-3-7e-staging-release-runbook-plan.md). |
| **P3-Demo-1** | Phase 0 + mock send path demo readiness verification | evidence/readiness | Verify the approved local/mock demo path is present and safe to show: secure Phase 0 foundation, tenant/auth/billing/access gates, campaign/contact flow, mock draft/evidence, human review, send-gate dry-run, mock send intent/outbound/audit, deliverability/outcomes read path, suppression/compliance gates, rate limits, Resend cold-outreach lockout, Stripe fail-closed skeleton, and no production/provider/money movement enablement. |
| **P3-Demo-2** | Local/mock demo login fix | fix/evidence | Fix browser demo login: stateful `MockAuthProvider` with `mockSignIn()`/`mockSignOut()`/localStorage; "Continue with Demo Account" button on `/login` (fail-closed in production); `X-Tenant-ID` seed fix in `TenantProvider`; 14 new tests. No real Clerk, no production JWT cutover, no real sending, no Stripe, no deployment, no AWS, no boot guard weakening. **Complete — 2026-06-30.** |
| **P3-8a** | Blocker consolidation and launch readiness dashboard | planning / docs | Docs-only. Consolidates all Phase 3 readiness for the boss (William): demo status (READY), completed slice summary, 38-row open blocker table with owners/values/risks, readiness categories (8 categories — demo READY, all real-provider/staging/production tracks BLOCKED), recommended next paths, risk notes, and boss-facing summary for direct delivery. No live providers, deployment, secrets, registry push, or production enablement. **Complete — 2026-06-30.** See [evidence/phase-3-8a-launch-readiness-dashboard.md](evidence/phase-3-8a-launch-readiness-dashboard.md). |
| **P3-Final** | Final boss handoff package | planning / docs | Docs-only. 9-section handoff for William: executive summary (14 checks), demo startup instructions (`docker compose up` → `http://localhost:3000/login` → demo button or credential form), 11-step browser flow, complete-work summary (Phase 0 through P3-8a with commits), intentionally-not-live list with kill switches, safety gate status for all invariants, open blockers (summarized from P3-8a), recommended next approval paths keyed to owner input, and boss-facing message for direct delivery. No live providers, deployment, secrets, registry push, or production enablement. **Complete — 2026-06-30.** See [evidence/phase-3-final-boss-handoff-package.md](evidence/phase-3-final-boss-handoff-package.md). |
| **P3-Audit** | Final Phase 3 requirements + architecture compliance audit | docs/evidence audit | Read-only audit verifies Phase 3 meets project requirements, locked architecture, owner decisions, safety/compliance rules, local/mock demo readiness, provider-disable constraints, and Phase 4 planning readiness. Backend/frontend gates and production Docker builds pass. Result: PASS — Phase 4 planning allowed only; provider/deployment work still blocked on owner values. **Complete — 2026-06-30.** See [evidence/phase-3-final-requirements-architecture-audit.md](evidence/phase-3-final-requirements-architecture-audit.md). |
| **P3-Audit-Cleanup** | Correct final handoff commit references | docs cleanup | Tiny docs-only cleanup correcting stale final handoff commit references: P3-Final package created at `9ec8d99`; final audited Phase 3 baseline is `747db3f`. No runtime behavior changed. **Complete — 2026-06-30.** |

Suggested execution order: P3-1 → P3-2 → P3-4 (done as rate-limit/abuse-protection hardening) → P3-5a–P3-5e docs-only provider-lane design + owner approval (Resend selected) → P3-7a deployment/ops readiness plan → P3-7b production Docker hardening/build smoke → P3-7c staging env/secret templates → P3-7d CI/CD plan → P3-7d-impl validation-only CI gates → P3-5f disabled Resend skeleton → P3-5g webhook verification foundation → P3-5h-prep internal-smoke checklist → P3-5i secret/readiness contract → P3-6a Stripe billing owner decision packet → P3-6b Stripe config/readiness contract → P3-6c Stripe owner defaults → P3-6d Stripe webhook foundation → P3-6e Stripe checkout/portal skeleton → **P3-5j dual sending-layer scope correction (architecture correction + send_layer guard + docs)** → **P3-Demo-1 Phase 0 + mock send path readiness verification** → P3-3 frontend Clerk when owner values are available → P3-7e staging release only after approval, with later real-adapter/internal-smoke/billing work only after their stop gates clear.
