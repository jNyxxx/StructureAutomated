...
| **P3-7** | Deployment runbook + staging | production readiness | **P3-7a inspection complete; P3-7b Docker hardening/build smoke complete; P3-7c staging env/secret template complete; P3-7d CI/CD release plan complete:** current dev Docker/Compose preserved; production backend/frontend images build; staging config map and secret-ref template exist; CI/CD plan defines validation, image tagging, approval gates, migration task, smoke evidence, and rollback/prod cutover rules. Staging/prod remain blocked on AWS account/region, deployment target, domains/TLS, Secrets Manager/KMS, RDS, Redis, backups/RPO/RTO, alerts, CI/CD implementation approval, migration/rollback owners, cutover approver, registry target, and Clerk staging values. Future P3-7e→P3-7g covers approved staging release, smoke evidence, and final production plan. **All boot-guard conditions must pass before any production boot.** |

Suggested execution order: P3-1 → P3-2 → P3-4 (done as rate-limit/abuse-protection hardening) → P3-5a–P3-5e docs-only provider-lane design + owner approval (Resend selected) → P3-7a deployment/ops readiness plan → P3-7b production Docker hardening/build smoke → P3-7c staging env/secret templates → P3-7d CI/CD plan → P3-3 frontend Clerk when owner values are available → P3-7e staging release only after approval, with P3-5f→P3-5h real-adapter work (and P3-6) only after their stop gates clear.
