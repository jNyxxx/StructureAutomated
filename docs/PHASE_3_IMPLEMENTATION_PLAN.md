...
| **P3-7** | Deployment runbook + staging | production readiness | **P3-7a inspection complete; P3-7b Docker hardening/build smoke complete; P3-7c staging env/secret template complete:** current dev Docker/Compose preserved; production backend/frontend images build; staging config map and secret-ref template exist in `docs/STAGING_ENVIRONMENT_TEMPLATE.md`. Staging/prod remain blocked on AWS account/region, deployment target, domains/TLS, Secrets Manager/KMS, RDS, Redis, backups/RPO/RTO, alerts, CI/CD approvals, migration/rollback owners, cutover approver, and Clerk staging values. Future P3-7d→P3-7g covers CI/CD plan, approved staging release, smoke evidence, and final production plan. **All boot-guard conditions must pass before any production boot.** |

Suggested execution order: P3-1 → P3-2 → P3-4 (done as rate-limit/abuse-protection hardening) → P3-5a–P3-5e docs-only provider-lane design + owner approval (Resend selected) → P3-7a deployment/ops readiness plan → P3-7b production Docker hardening/build smoke → P3-7c staging env/secret templates → P3-3 frontend Clerk when owner values are available → P3-7d CI/CD plan, with P3-5f→P3-5h real-adapter work (and P3-6) only after their stop gates clear.
