...
| **P3-6** | Stripe / payment | real provider integration — **DEFERRED** | **P3-6a owner decision packet created, unanswered; P3-6b config/readiness contract created. STOP GATE remains.** Requires written owner approval and first-paying-client billing decisions: provider, test/live mode, products/prices, entitlements, access rules, webhooks, dunning, refund/chargeback policy, config refs, and billing owners. No Stripe implementation, SDK/API call, checkout, webhook, real billing, or money movement until a later approved slice. See [evidence/phase-3-6b-stripe-config-secret-readiness-contract.md](evidence/phase-3-6b-stripe-config-secret-readiness-contract.md). |
...
| Stripe / payment approval | P3-6 | Open — P3-6a decision packet and P3-6b readiness contract created; owner answers still required before implementation |
