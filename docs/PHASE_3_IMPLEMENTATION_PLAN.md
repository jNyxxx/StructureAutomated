...
| **P3-6** | Stripe / payment | real provider integration — **DEFERRED** | **P3-6a owner decision packet created; P3-6b config/readiness contract created; P3-6c safe defaults recorded. STOP GATE remains for implementation.** Defaults select Stripe direction, test mode first, mock billing default, manual-first-pilot option, self-serve checkout disabled by default, placeholder internal plans, 14-day trial, access defaults, 7-day payment failure grace, webhook event defaults, and owner/operator fallback roles. Still missing final public prices, usage limits, named owners, test refs/price IDs, smoke approval, and live-mode approval. No Stripe implementation, SDK/API call, checkout, webhook, real billing, or money movement until a later approved slice. See [evidence/phase-3-6c-stripe-owner-defaults.md](evidence/phase-3-6c-stripe-owner-defaults.md). |
...
| Stripe / payment approval | P3-6 | Partially answered by P3-6c safe defaults; still open for exact final values, named owners, test refs/price IDs, smoke approval, and live-mode approval before implementation |
