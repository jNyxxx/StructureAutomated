...
| **P3-7** | Deployment runbook + staging | production readiness | **P3-7a docs-only inspection complete; P3-7b Docker hardening complete with Docker-daemon build limit:** current dev Docker/Compose preserved; production-specific backend/frontend Dockerfiles added; backend/frontend `.dockerignore` hardened; Next standalone output enabled; backend/frontend gates passed. Docker image build validation must be rerun after Docker Desktop/Linux engine is available. Staging/prod remain blocked on AWS account/region, deployment target, domains/TLS, Secrets Manager/KMS, RDS, Redis, backups/RPO/RTO, alerts, CI/CD approvals, migration/rollback owners, and cutover approver. Future P3-7c→P3-7g covers staging env/secret templates, CI/CD plan, approved staging release, smoke evidence, and final production plan. **All boot-guard conditions must pass before any production boot.** |

Suggested execution order: P3-1 → P3-2 → P3-4 (done as rate-limit/abuse-protection hardening) → P3-5a–P3-5e docs-only provider-lane design + owner approval (Resend selected) → P3-7a deployment/ops readiness plan → P3-7b production Docker hardening → P3-3 frontend Clerk when owner values are available → P3-7c staging env/secret templates, with P3-5f→P3-5h real-adapter work (and P3-6) only after their stop gates clear.
