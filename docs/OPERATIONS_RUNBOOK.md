...
P4-FinalManualDemoSmoke (2026-07-02): final local/mock demo smoke evidence recorded as BLOCKED for final sign-off. Core Docker demo flow passed through contact import, campaign create, grounded draft generation, review approval, send-gate, mock send, outbound record, audit trail, billing/access, compliance/suppressions, deliverability, and outcomes. Required logout -> login cycle failed after `POST /auth/logout`: the local mock auth service revokes the fixed `local_mock_session_ref`, and both demo tokens then return `401 AUTH_SESSION_REVOKED` until the backend process is restarted or local mock session handling is fixed. Before including sign-out/sign-in in the boss walkthrough, open **P4-LocalMockAuthSessionCycle-Fix** or remove that step from the demo script. See [evidence/phase-4-final-manual-demo-smoke.md](evidence/phase-4-final-manual-demo-smoke.md).
