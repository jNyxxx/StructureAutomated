...
_REVIEW_ITEM_COLUMNS = (
    ReviewItem.id,
    ReviewItem.tenant_id,
    ReviewItem.draft_id,
    ReviewItem.campaign_id,
    ReviewItem.contact_id,
    ReviewItem.status,
    ReviewItem.reviewer_user_id,
    ReviewItem.action_reason,
...
def _review_item(row: RowMapping) -> ReviewRecord:
    return ReviewRecord(
        id=row["id"],
...
class ReviewRepository(BaseRepository):
        ...
        row = (
            (
                await self.conn.execute(
                    ...
                    .returning(*_REVIEW_ITEM_COLUMNS)
                )
            )
            .mappings()
            .one()
    ...
    async def get_review_item(
        ...
        row = (
            (
                await self.conn.execute(
                    select(*_REVIEW_ITEM_COLUMNS).where(
    ...
    async def list_review_queue(
        ...
        *,
        ...
        stmt = select(*_REVIEW_ITEM_COLUMNS).where(ReviewItem.tenant_id == tenant_id)
