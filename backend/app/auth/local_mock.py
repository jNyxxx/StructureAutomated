...
_PROVIDER_SESSION_REF = "local_mock_session_ref"
_LOCAL_TOKENS = ("token-sentinel", "fake-valid-token")  # noqa: S105 - fake local/mock tokens only
...
class _LocalMockSessions:
    ...
    async def revoke_all_for_user(self, *, user_id: uuid.UUID, revoked_at: datetime) -> int:
        self._revoked.add(_PROVIDER_SESSION_REF)
        return 1
...
def build_local_mock_auth_service() -> AuthService:
    ...
    verifier = LocalMockClerkVerifier(
        {
            token: VerifiedClerkClaims(
                provider_user_id=_PROVIDER_USER_ID,
                email="owner@example.com",
                provider_session_ref=_PROVIDER_SESSION_REF,
                expires_at=1_893_456_000,
                mfa_verified=True,
            )
            for token in _LOCAL_TOKENS
        }
    )
    return AuthService(
        verifier=verifier,
