...
import { mailboxHealth } from "./deliverability-sample-data";

export function MailboxHealthCard() {
  return (
    <BentoCard title="Mailbox health" description="Read-only local mailbox preview. No provider integration or real sending." badge="Demo">
        ...
        {mailboxHealth.map((mailbox) => (
          <div key={mailbox.label} className="rounded-medium border border-border bg-panel2 p-4">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              ...
              <GateReasonBadge state={mailbox.status === "blocked" ? "blocked" : "warning"} label={mailbox.status} />
            </div>
            <div className="mt-3 grid gap-3 sm:grid-cols-3">
              <div className="rounded-small border border-border bg-panel p-2">
                <p className="text-caption text-subtle">Daily limit</p>
              ...
              <div className="rounded-small border border-border bg-panel p-2">
                <p className="text-caption text-subtle">Warmup day</p>
